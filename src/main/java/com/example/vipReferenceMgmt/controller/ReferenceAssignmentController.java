...
public class ReferenceAssignmentController {
	...
	public ResponseEntity<List<VipReferenceListResponse>> getFilteredReferences(
		...
		List<VipReferenceListResponse> responseList = referenceAssignmentService
				.getReferencesByLoginIdAndQueueAndStatus(request);
	...
	@PostMapping("/forward-reference")
	public ResponseEntity<String> forwardReference(@RequestBody ForwardReference request){
		return referenceAssignmentService.forwardReference(request);
	}
}
